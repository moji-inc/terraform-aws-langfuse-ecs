...
aws ecs update-service \
  ...
  --region "${STAGING_REGION}" \
  --cluster "${STAGING_CLUSTER}" \
  --service "${STAGING_WEB_SERVICE}" \
  --task-definition "${STAGING_WEB_TASK}" \
  --desired-count 1 \
  --output json >/dev/null

echo "Waiting for the staging web service to become stable..."
aws ecs wait services-stable \
  --profile "${PROFILE_NAME}" \
  --region "${STAGING_REGION}" \
  --cluster "${STAGING_CLUSTER}" \
  --services "${STAGING_WEB_SERVICE}"

aws ecs update-service \
  --profile "${PROFILE_NAME}" \
  --region "${STAGING_REGION}" \
  --cluster "${STAGING_CLUSTER}" \
  --service "${STAGING_WORKER_SERVICE}" \
  --task-definition "${STAGING_WORKER_TASK}" \
  --desired-count 1 \
  --output json >/dev/null

echo "Waiting for the staging worker service to become stable..."
aws ecs wait services-stable \
  --profile "${PROFILE_NAME}" \
  --region "${STAGING_REGION}" \
  --cluster "${STAGING_CLUSTER}" \
  --services "${STAGING_WORKER_SERVICE}"
