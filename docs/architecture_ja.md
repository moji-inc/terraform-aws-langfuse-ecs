...
└──────────────────────────────┬──────────────────────────────────────┘
                        ...
                        └──────┬──────┘
                               │ 3000/tcp
                  ┌────────────▼────────────┐
                  │        Web SG           │  ALB有効時: ALBからのみ
                  │   (Private w/ ALB)      │  ALB無効時: allowed_cidrsから直接
                  └────────────┬────────────┘
                               │
         ┌─────────────────────┼─────────────────────┐
         │                     │                     │
   ...
   └─────┬─────┘        │ ← Web/Worker│       └─────────────┘
         ...
         │
   ┌─────▼─────┐
   │  RDS SG   │  5432/tcp ← Web/Worker のみ
   └───────────┘
```

#### Security Group ルール

| SG Name | Inbound Rule | Source | Egress | 説明 |
|---|---|---|---|---|
| `sg-alb` | TCP 443, 80 | `var.allowed_cidrs`（HTTP+HTTPS）、`var.allowed_security_group_ids`（HTTPS のみ） | All | HTTPS 終端用 ALB |
| `sg-web` | TCP 3000 | sg-alb (ALB有効時) または `var.allowed_cidrs` (ALB無効時) | All | Langfuse Web アプリケーション |
| `sg-worker` | TCP 3030 | sg-web | All | Worker ヘルスチェックエンドポイント |
| `sg-clickhouse` | TCP 8123, 9000 | sg-web, sg-worker | All | ClickHouse HTTP/Native プロトコル |
| `sg-rds` | TCP 5432 | sg-web, sg-worker | None | PostgreSQL（Egress 不要） |
| `sg-redis` | TCP 6379 | sg-web, sg-worker | None | Redis キャッシュ・キュー |
| `sg-efs` | TCP 2049 | sg-clickhouse | None | ClickHouse データ用 EFS マウント |
| `sg-vpc-endpoints` | TCP 443 | VPC CIDR | None | VPC Endpoints (ECR, Logs, Secrets Manager) |

#### セキュリティ設計の補足

- **ALB Security Group**: `enable_alb = true` の場合のみ作成。指定した IP 範囲 (`allowed_cidrs`) および/またはセキュリティグループ (`allowed_security_group_ids`) からのアクセスに制限。
- **Web Security Group**: ALB 有効時は `allowed_cidrs` からの直接アクセスを無効化。ALB 経由のみ許可。
- **RDS Security Group**: Egress ルールなし（RDS マネージドサービスはアウトバウンド接続不要）。
- **最小権限の原則**: 各コンポーネントは必要なサービスとのみ通信可能。
...
| `enable_alb` | `bool` | ALB を有効化して HTTPS アクセス (default: `true`) |
| `certificate_arn` | `string` | HTTPS 用 ACM 証明書 ARN。未指定時は自己署名証明書を自動生成。 |
| `custom_domain` | `string` | カスタムドメイン (例: `langfuse.example.com`)。オプション。 |
| `route53_zone_id` | `string` | Route53 ホストゾーン ID。custom_domain 指定時は必須。 |
...
ALB はデフォルトで有効 (`enable_alb = true`)。HTTPS アクセスと自動証明書管理を提供。

### 証明書オプション

| 設定 | 証明書 | アクセス URL |
|---|---|---|
| デフォルト (`certificate_arn` なし) | 自己署名証明書（自動生成） | `https://<alb-dns-name>` (ブラウザ警告あり) |
| `certificate_arn` 指定 | ACM 証明書 | `https://<alb-dns-name>` またはカスタムドメイン |
| `custom_domain` + `route53_zone_id` 指定 | ACM 証明書 | `https://langfuse.example.com` |

### オプション A: 自己署名証明書（デフォルト、最も簡単）

```hcl
enable_alb = true
# certificate_arn 未設定 - 自己署名証明書が自動生成される
```

アクセス: `https://<alb-dns-name>` (ブラウザにセキュリティ警告が表示される)

**注意**: 自己署名証明書は開発・テスト用途に適している。本番環境では ACM 証明書 + カスタムドメインを推奨。

### オプション B: ACM 証明書（本番環境推奨）
...
### オプション C: カスタムドメイン + Route53（DNS 完全自動化）
...
Terraform が Route53 に A レコード（エイリアス）を自動作成。

### ALB 有効時の動作

ALB 有効時:
- Langfuse Web は Private Subnet に移動（Public IP なし）
- HTTPS:443 は常に有効（ACM または自己署名証明書）
- HTTP:80 と HTTPS:443 の両方がターゲットグループにフォワード（両プロトコルでアクセス可能）
- 通信経路: Internet → ALB (HTTPS) → ECS (HTTP:3000)
